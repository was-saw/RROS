...
impl file_operations::FileOpener<u8> for HecticFile {
    fn open(_context: &u8, file: &kernel::file::File) -> kernel::Result<Self::Wrapper> {
        ...
        let ctx_ref = Arc::get_mut(&mut ctx).unwrap().get_mut();
        ctx_ref.init()?;
...
impl file_operations::FileOperations for HecticFile {
    ...
    type Wrapper = Arc<UnsafeCell<RtswitchContext>>;
    ...
    fn ioctl(
        ...
        match cmd.cmd {
            RROS_HECIOC_LOCK_STAX => match ctx.stax.as_ref().get_ref().lock() {
                Ok(g) => {
                    let tmp = Box::try_new(g)?;
                    ctx.o_guard.try_push(Box::into_raw(tmp) as usize)?;
                }
                Err(e) => {
                    return Err(e);
                }
            ...
            RROS_HECIOC_UNLOCK_STAX => {
                if let Some(tmp) = ctx.o_guard.pop() {
                    unsafe {
    ...
    fn oob_ioctl(
        ...
        match cmd.cmd {
            RROS_HECIOC_LOCK_STAX => match ctx.stax.as_ref().get_ref().lock() {
                Ok(g) => {
                    let tmp = Box::try_new(g)?;
                    ctx.i_guard.try_push(Box::into_raw(tmp) as usize)?;
                }
                Err(e) => {
                    return Err(e);
                }
            ...
            RROS_HECIOC_UNLOCK_STAX => {
                if let Some(tmp) = ctx.i_guard.pop() {
                    unsafe {
                        Box::from_raw(tmp as *mut Guard<'_, Stax<()>>);
                    }
                }
